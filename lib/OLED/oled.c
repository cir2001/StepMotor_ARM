...
/**
 * @brief 在 OLED 上显示带有自定义前缀的十六进制数值
 * @param x 起始 X 坐标
 * @param y 起始 Y 坐标
 * @param title_prefix 要显示的前缀字符串 (例如 "CAN_ID:", "ID_FB:", "Temp:")
 * @param value 要显示的数值 (uint32_t)
 */
void OLED_ShowString_Data(uint8_t x, uint8_t y, const char *title_prefix, uint32_t value)
{
    // 缓冲区大小：例如 "Temp:0x1FFFFFFF" (20个字符通常够用)
    char display_buffer[25]; 
    int len;

    // 格式化输出：将 "前缀"、"0x" 和 十六进制数值写入缓冲区
    // %s 用于插入字符串 title_prefix
    // %03lX 用于插入数值 value
    
    // 1. 格式化数据
    len = sprintf(display_buffer, "%s0x%03lX", title_prefix, (unsigned long)value);
    ...
    // 2. 显示字符串
    OLED_ShowString(x, y, (unsigned char *)display_buffer);
}
